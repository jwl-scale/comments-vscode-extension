...
export function activate(context: vscode.ExtensionContext): MdCommentsApi {
  ...
  /**
   * Cursor never shows a session id anywhere in its UI — the id lives only in
   * the transcript path (~/.cursor/projects/<slug>/agent-transcripts/<id>/) and
   * in its state DB. So list the chats instead of asking the user to find one.
   *
   * The transcript directory is the source of truth (a chat with no transcript
   * cannot be deeplinked anyway); names come from the state DB when readable.
   */
  cmd('mdComments.cursorChatRef', async () => {
    const sessions = findLocalSessions(1000).filter((s) => s.scheme === 'cursor');
    if (sessions.length === 0) {
      vscode.window.showWarningMessage(
        'Comments: no Cursor chats with transcripts found under ~/.cursor/projects/*/agent-transcripts/.',
      );
      return;
    }
    const inCursor = (await vscode.commands.getCommands(true)).includes(CURSOR_OPEN_CHAT_COMMAND);
    const openButton = { iconPath: new vscode.ThemeIcon('comment-discussion'), tooltip: 'Open this chat in Cursor' };

    const picker = vscode.window.createQuickPick<vscode.QuickPickItem & { session: LocalSession }>();
    picker.title = 'Cursor chats';
    picker.placeholder = 'Pick a chat — Enter copies its cursor: ref' + (inCursor ? ', 💬 opens it in Cursor' : '');
    picker.matchOnDescription = true;
    picker.matchOnDetail = true;
    picker.items = sessions.map((session) => ({
      label: session.preview || session.sessionId,
      description: session.sessionId.slice(0, 8),
      detail: new Date(session.mtime).toLocaleString(),
      buttons: inCursor ? [openButton] : [],
      session,
    }));
    picker.onDidTriggerItemButton(async (e) => {
      picker.hide();
      await vscode.commands.executeCommand('mdComments.openCursorChat', e.item.session.sessionId);
    });
    picker.onDidAccept(async () => {
      const item = picker.selectedItems[0];
      picker.hide();
      if (!item) return;
      const ref = `cursor:${item.session.sessionId}`; // scheme-literal-ok: this picker only lists Cursor chats
      await vscode.env.clipboard.writeText(ref);
      const choice = await vscode.window.showInformationMessage(
        `Copied "${ref}" — paste it into a comment.`,
        ...(inCursor ? ['Open in Cursor'] : []),
        'Open conversation',
      );
      if (choice === 'Open in Cursor') {
        await vscode.commands.executeCommand('mdComments.openCursorChat', item.session.sessionId);
      } else if (choice === 'Open conversation') {
        await vscode.commands.executeCommand('mdComments.openConversation', item.session.sessionId, null, 'cursor');
      }
    });
    picker.onDidHide(() => picker.dispose());
    picker.show();
  ...
  /**
   * Jump from a ref to the live chat in Cursor's own UI. Cursor exposes no URL
   * route for this, only an internal command, so it works exactly when the
   * extension host is Cursor.
   */
  cmd('mdComments.openCursorChat', async (sessionId?: string) => {
    if (!sessionId) return;
    if (!(await vscode.commands.getCommands(true)).includes(CURSOR_OPEN_CHAT_COMMAND)) {
      vscode.window.showWarningMessage(
        'Comments: opening a Cursor chat requires running inside Cursor — this window is ' +
          `${vscode.env.appName}. The conversation is still viewable here via the graph.`,
      );
      return;
    }
    try {
      await vscode.commands.executeCommand(CURSOR_OPEN_CHAT_COMMAND, sessionId);
    } catch (err) {
      vscode.window.showErrorMessage(`Comments: Cursor refused to open that chat — ${(err as Error).message}`);
    }
  });
