...
export function activate(context: vscode.ExtensionContext): MdCommentsApi {
  ...
  cmd('mdComments.showThreadSessions', async (thread?: ThreadArg) => {
    ...
    const copyButton = { iconPath: new vscode.ThemeIcon('copy'), tooltip: 'Copy claude: ref' };
    ...
    for (const s of sessions) {
      ...
      items.push({
        ...
        detail: `claude:${s.sessionId}`,
        ref: `claude:${s.sessionId}`,
        buttons: [copyButton],
      ...
      for (const c of s.comments) {
        ...
        const ref = segment
          ? segment.from === segment.to
            ? `claude:${s.sessionId}#${segment.to}`
            : `claude:${s.sessionId}#${segment.from}..${segment.to}`
          : `claude:${s.sessionId}`;
        const focus = segment
          ? segment.from === segment.to
  ...
  cmd(
    ...
    async (sessionId?: string, focus?: ConversationFocus | string | null) => {
      const dir = store.defaultSessionsDir();
      if (!dir) {
      ...
      const local = findLocalSessions(500).find((s) => s.sessionId === sessionId);
  ...
  cmd('mdComments.attachAgentSession', async (thread?: vscode.CommentThread) => {
    ...
    if (sessions.length === 0) {
      vscode.window.showWarningMessage('Comments: no local agent sessions found (~/.claude/projects, $CODEX_HOME/sessions).');
      return;
    }
    const pick = await vscode.window.showQuickPick(
      sessions.map((s) => ({
    ...
    const ref = `claude:${pick.session.sessionId}`;
    if (thread) {
