...
import { AGENT_SCHEMES, AgentScheme, Mention, parseMentions, parseSessionRef } from './refs';
...
export function planFromThread(state: ThreadState, agents: AgentDefinition[]): RunPlan {
  ...
  // A bare provider mention (@claude, @codex, @cursor) selects a provider, not
  // an agent DEFINITION — never look one of those up in .claude/agents/.
  const agentMention = mentions.find(
    (m) => m.kind === 'agent' && !(AGENT_SCHEMES as readonly string[]).includes(m.name),
  );
  return {
...
export async function runAgentOnThread(
  ...
  if (provider === 'codex') {
    ...
    const knownRef = built.knownSessionId ? `${provider}:${built.knownSessionId}` : undefined;
    if (knownRef) {
      ...
      mcpServers: {
        comments: {
          ...
          env: {
            ...
            ...(decision.knownSessionId ? { MD_COMMENTS_SESSION: `${provider}:${decision.knownSessionId}` } : {}),
          },
        },
      },
    });
    ...
    const onLine = (line: string) => {
      ...
      if (provider === 'codex') {
        ...
        if (found && found !== sessionId) {
          ...
          if (sessionFile) {
            try {
              fs.writeFileSync(sessionFile, `${provider}:${found}`);
            } catch {
