...
import { Mention, parseMentions } from './refs';
...
/** Steering: mentions in the newest comment win; falls back to plain claude. */
export function planFromThread(state: ThreadState, agents: AgentDefinition[]): RunPlan {
  ...
  const agentMention = mentions.find((m) => m.kind === 'agent' && m.name !== 'claude');
  return {
...
export async function runAgentOnThread(
  ...
  const buildParams = {
    ...
    mentionSessionId: plan.forkSessionId,
    threadLastSessionId: lastSessionId(state.events),
  ...
  if (provider === 'codex') {
    // Codex takes MCP servers as config overrides, and cannot be handed a
    // session id up front — so MD_COMMENTS_SESSION is unset for fresh runs and
    // the id is recovered from the --json stream instead. The skill tells the
    // agent to call register_session itself, which covers the gap.
    ...
    args = [
      ...
      ...(built.knownSessionId
        ? ['-c', `mcp_servers.comments.env.MD_COMMENTS_SESSION="${built.knownSessionId}"`]
        : []),
      ...
      mcpServers: {
        comments: {
          ...
          env: {
            ...
            ...(decision.knownSessionId ? { MD_COMMENTS_SESSION: decision.knownSessionId } : {}),
          },
        },
      },
    });
  ...
  const result = await new Promise<AgentRunResult>((resolve) => {
    ...
    const onLine = (line: string) => {
      ...
      if (provider === 'codex') {
        sessionId = codexSessionIdFromEvent(line) ?? sessionId;
