    ...
    this.panel.webview.onDidReceiveMessage(async (msg) => {
      if (msg.type === 'ready') {
        ...
        const ref = msg.agentId
          ? `claude:${this.graph.sessionId}@${msg.agentId}`
          : msg.rangeEnd && msg.uuid
            ? `claude:${this.graph.sessionId}#${msg.uuid}..${msg.rangeEnd}`
            : msg.uuid
              ? `claude:${this.graph.sessionId}#${msg.uuid}`
              : `claude:${this.graph.sessionId}`;
