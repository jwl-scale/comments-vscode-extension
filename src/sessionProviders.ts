...
export function cursorProjectsDir(): string {
  // MD_COMMENTS_CURSOR_HOME exists so this is testable against a fixture tree —
  // without it, directory discovery can only be exercised against whatever
  // happens to be in the developer's home directory, which is why it went
  // untested initially.
  const home = process.env.MD_COMMENTS_CURSOR_HOME || path.join(os.homedir(), '.cursor');
...
// ---------- cursor: names and request-id lookup (SQLite state) ----------

/**
 * Cursor keeps chat metadata in its VS Code-style state DB, separately from the
 * transcripts on disk. Two things there are worth reading, both keyed by
 * `composerId` — which IS the transcript session id:
 *
 *   - `composer.composerHeaders` → the chat's human-readable name, the same one
 *     shown in Cursor's sidebar. Far better than guessing from the first prompt.
 *   - `bubbleId:<composerId>:<bubbleId>` records carry a `requestId`, which is
 *     what Cursor's "Copy Request ID" puts on the clipboard. Resolving it is the
 *     only way to get from something the UI offers to a session id.
 *
 * Read through the `sqlite3` CLI in readonly mode rather than taking a native
 * dependency: the extension bundle must stay dependency-free and must never
 * risk locking a database Cursor is actively writing. Every function here
 * degrades to empty rather than throwing — this is a convenience layer, and
 * transcripts on disk remain the source of truth.
 */
/**
 * Cursor's state DB lives wherever VS Code-family editors put user data, which
 * differs per platform. Getting this wrong fails silently — names just never
 * load — so it is resolved explicitly rather than assumed.
 */
export function cursorStateDb(): string {
  const override = process.env.MD_COMMENTS_CURSOR_STATE_DB;
  ...
  if (process.platform === 'win32') {
    return path.join(process.env.APPDATA || path.join(os.homedir(), 'AppData', 'Roaming'), ...rel);
  }
  // Linux and everything else follow the XDG default VS Code uses.
  return path.join(process.env.XDG_CONFIG_HOME || path.join(os.homedir(), '.config'), ...rel);
}

function querySqlite(db: string, sql: string): string | null {
  if (!fs.existsSync(db)) return null;
  try {
    const res = spawnSync('sqlite3', ['-readonly', db, sql], {
      encoding: 'utf8',
      timeout: 5000,
    ...
    return null; // no sqlite3 on PATH (Windows) — callers fall back
  }
}

/** Parse `composer.composerHeaders` into composerId → chat name. Pure. */
export function parseComposerHeaders(json: string): Map<string, string> {
  const out = new Map<string, string>();
  try {
    for (const c of JSON.parse(json)?.allComposers ?? []) {
      if (c?.composerId && c?.name) out.set(String(c.composerId), String(c.name));
    }
  } catch {
    /* malformed or absent */
  }
...
/**
 * Cursor's own command for focusing a chat, discovered from its bundle:
 * `executeCommand("composer.openComposer", composerId)`. composerId IS our
 * session id, so a `cursor:` ref maps onto it directly.
 *
 * Only present when the extension host IS Cursor — in VS Code the command does
 * not exist, and there is no URL-scheme route for it either (Cursor registers
 * cursor-deeplink only for automations and background-agent).
 */
export const CURSOR_OPEN_CHAT_COMMAND = 'composer.openComposer';

/** composerId → the chat name Cursor shows in its sidebar. */
export function cursorChatNames(): Map<string, string> {
  const raw = querySqlite(
    cursorStateDb(),
    "SELECT value FROM ItemTable WHERE key='composer.composerHeaders';",
  );
  return raw ? parseComposerHeaders(raw) : new Map();
}

/**
 * Resolve a Cursor "request ID" (what the UI's Copy Request ID yields) to the
 * session id, by finding the bubble that recorded it: keys are
 * `bubbleId:<composerId>:<bubbleId>`.
 */
export function resolveCursorRequestId(requestId: string): string | null {
  if (!/^[0-9a-fA-F-]{36}$/.test(requestId)) return null;
  const rows = querySqlite(
    cursorStateDb(),
    `SELECT substr(key, 10, 36) FROM cursorDiskKV WHERE key LIKE 'bubbleId:%' ` +
      `AND json_extract(value,'$.requestId')='${requestId}' LIMIT 1;`,
  );
  const id = (rows ?? '').trim().split('\n')[0];
