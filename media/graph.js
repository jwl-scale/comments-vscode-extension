...
(function () {
  ...
  window.addEventListener('message', (e) => {
    ...
    if (msg.type === 'load') {
      ...
      state.canOpenInCursor = !!msg.canOpenInCursor;
  ...
  function messageCard(m, isFork) {
    ...
    card.append(
      el('div', { class: 'head' },
        ...
        el('span', {
          class: 'copy',
          title: 'Copy the deeplink for this message',
  ...
  function render() {
    ...
    // Jump to the live chat in Cursor's own UI. Only rendered when the host can
    // actually do it (a Cursor conversation, viewed inside Cursor).
    if (state.canOpenInCursor) {
      toolbar.append(
        el('button', {
          class: 'tbtn',
          title: 'Open this conversation in the Cursor chat pane',
          onclick: () => vscode.postMessage({ type: 'openInCursor' }),
        }, '💬 Open in Cursor'),
      );
    }
    if (state.view === 'graph') {
