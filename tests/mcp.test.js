...
/** Bare workspace with one source file — enough for thread anchoring. */
...
/** Fold-free read: these assertions are about the raw appended events. */
function readThread(root, threadId) {
  const file = path.join(root, '.comments', 'threads', `${threadId}.jsonl`);
  const events = fs
...
// ---------- late-bound session stamping (codex) ----------

test('stamping: MD_COMMENTS_SESSION_FILE binds the session after the server starts', async () => {
  const root = mkFixtureRepo();
  const sessionFile = path.join(os.tmpdir(), `mdc-test-session-${Date.now()}`);
  let server;
  try {
    // Codex cannot be handed an id up front, so the server starts with the file
    // absent — exactly the state a fresh run begins in.
    server = startServer(root, { MD_COMMENTS_SESSION_FILE: sessionFile });
    await server.request('initialize', { protocolVersion: '2024-11-05', capabilities: {} });

    const a = callResult(
      await server.request('tools/call', {
        name: 'create_thread',
        arguments: { file: 'src/app.ts', anchorText: 'return 42;', body: 'before the id is known' },
      }),
    );
    // Nothing to stamp yet: better unstamped than stamped with a guess.
    const before = readThread(root, a.threadId);
    assert.equal(before.events[0].actor.session, undefined);

    // The run announces itself; the runner fills the file.
    fs.writeFileSync(sessionFile, 'codex:01a01375-7946-7fb2-a4ce-84248963a364');

    const b = callResult(
      await server.request('tools/call', {
        name: 'reply_to_thread',
        arguments: { threadId: a.threadId, body: 'after the id is known' },
      }),
    );
    assert.ok(b);
    const after = readThread(root, a.threadId);
    const replied = after.events.find((e) => e.type === 'replied');
    // Re-read per write is what makes this work; a cached lookup would still
    // be empty here.
    assert.equal(replied.actor.session, 'codex:01a01375-7946-7fb2-a4ce-84248963a364');
  } finally {
    server?.close();
...
test('stamping: a half-written or junk session file is never stamped', async () => {
  const root = mkFixtureRepo();
  const sessionFile = path.join(os.tmpdir(), `mdc-test-junk-${Date.now()}`);
  let server;
  try {
    fs.writeFileSync(sessionFile, 'not a valid ref at all');
    server = startServer(root, { MD_COMMENTS_SESSION_FILE: sessionFile });
    await server.request('initialize', { protocolVersion: '2024-11-05', capabilities: {} });
    const a = callResult(
      await server.request('tools/call', {
        name: 'create_thread',
        arguments: { file: 'src/app.ts', anchorText: 'return 42;', body: 'junk file' },
      }),
    );
    // An event, once appended, cannot be taken back — so refuse to stamp
    // anything that does not parse as a ref.
    assert.equal(readThread(root, a.threadId).events[0].actor.session, undefined);
  } finally {
    server?.close();
    fs.rmSync(sessionFile, { force: true });
    fs.rmSync(root, { recursive: true, force: true });
