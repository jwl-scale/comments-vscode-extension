...
test('codex: session id is recovered from the --json event stream', () => {
  // The real shape, and the FIRST line codex emits (codex-cli 0.146.1). Getting
  // this wrong means the id is never learned and no write is ever stamped.
  assert.equal(
    codexSessionIdFromEvent('{"type":"thread.started","thread_id":"01a01375-7946-7fb2-a4ce-84248963a364"}'),
    '01a01375-7946-7fb2-a4ce-84248963a364',
  );
  // Defensive fallbacks across codex-cli versions.
  assert.equal(
  ...
  // Non-id events on the same stream must not be mistaken for the session.
  assert.equal(codexSessionIdFromEvent('{"type":"turn.started"}'), undefined);
  assert.equal(
    codexSessionIdFromEvent('{"type":"item.completed","item":{"id":"item_0","type":"agent_message"}}'),
    undefined,
  );
