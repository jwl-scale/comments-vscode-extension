...
const { AGENT_REF } = require('../out/refs.js');

test('agent refs: all three schemes linkify, unknown ones do not', () => {
  const found = (s) => [...s.matchAll(AGENT_REF)].map((m) => `${m[1]}:${m[2]}${m[3] ? '#' + m[3] : ''}`);

  assert.deepEqual(found('see cursor:8c6e09e8-ef50-443c-a5b0-ebf683645acc for context'), [
    'cursor:8c6e09e8-ef50-443c-a5b0-ebf683645acc',
  ]);
  // Cursor message ids are synthesized positions, not uuids — they must still
  // parse as a focus.
  assert.deepEqual(found('cursor:sess-a#m12'), ['cursor:sess-a#m12']);
  assert.deepEqual(found('claude:a#u1 and codex:b#i2 and cursor:c#m3'), [
    'claude:a#u1',
    'codex:b#i2',
    'cursor:c#m3',
  ]);
  // An unregistered scheme is prose, not a link.
  assert.deepEqual(found('gemini:abc123 is not ours'), []);
});
