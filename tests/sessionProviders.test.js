...
// ---------- ref construction must never hardcode a provider ----------

test('no source file builds a session ref by hardcoding a scheme', () => {
  // Every ref-producing UI path once concatenated `claude:` + id, so attaching
  // a codex or cursor session handed you a ref that could never resolve. The
  // scheme must come from the session, never from a literal.
  ...
  for (const name of fs.readdirSync(srcDir)) {
    ...
    text.split('\n').forEach((line, i) => {
      // A scheme literal immediately followed by an interpolated id. A line may
      // opt out with `scheme-literal-ok:` PLUS a reason — provider-specific
      // commands legitimately name their provider — but it must be deliberate.
      if (/`(claude|codex|cursor):\$\{/.test(line) && !/scheme-literal-ok:\s*\S/.test(line)) {
        offenders.push(`${name}:${i + 1}  ${line.trim()}`);
      }
    });
  }
  assert.deepEqual(offenders, [], `hardcoded scheme in ref construction:\n${offenders.join('\n')}`);
});

test('a graph carries the provider that produced it', () => {
  // graphPanel builds "copy deeplink" refs from graph.scheme, so a graph
  // without one silently reintroduces the hardcoded-claude bug.
  const dir = tmpdir();
  try {
    const codexFile = path.join(dir, 'x.jsonl');
    writeJsonl(codexFile, [
      CODEX_META,
      { type: 'response_item', payload: { type: 'message', id: 'm1', role: 'user', content: [{ type: 'input_text', text: 'hi' }] } },
    ]);
    assert.equal(loadCodexGraph(codexFile, 's1').scheme, 'codex');

    const cursorFile = path.join(dir, 'y.jsonl');
    writeJsonl(cursorFile, [cursorEntry('user', [{ type: 'text', text: 'hi' }])]);
    assert.equal(loadCursorGraph(cursorFile, 's2').scheme, 'cursor');
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
});

test('cursor: composer headers map session ids to the names Cursor shows', () => {
  // The picker uses these instead of raw prompt text, because the sidebar name
  // is what a user is actually looking at when hunting for a chat.
  const { parseComposerHeaders } = require('../out/sessionProviders.js');
  const headers = JSON.stringify({
    allComposers: [
      { composerId: 'f57e9de4-523c-47b7-b8d3-6c5252e27ce5', name: 'Causation based reasoning' },
      { composerId: 'no-name-here' },
    ],
  });
  const names = parseComposerHeaders(headers);
  assert.equal(names.get('f57e9de4-523c-47b7-b8d3-6c5252e27ce5'), 'Causation based reasoning');
  assert.equal(names.has('no-name-here'), false);
  // Never throw on a missing or malformed state DB — it is a convenience layer.
  assert.equal(parseComposerHeaders('not json').size, 0);
  assert.equal(parseComposerHeaders('{}').size, 0);
});

// ---------- cursor: discovery, portability, degradation ----------

const { cursorStateDb, findLocalCursorSessions, locateCursorSession } = require('../out/sessionProviders.js');

/** Build a real ~/.cursor tree: projects/<slug>/agent-transcripts/<id>/<id>.jsonl */
function makeCursorHome(sessions) {
  const home = tmpdir('mdc-cursor-home-');
  for (const { slug, id, entries } of sessions) {
    const dir = path.join(home, 'projects', slug, 'agent-transcripts', id);
    writeJsonl(path.join(dir, `${id}.jsonl`), entries ?? [cursorEntry('user', [{ type: 'text', text: 'hi' }])]);
  }
  return home;
}

function withCursorHome(home, fn) {
  const prev = process.env.MD_COMMENTS_CURSOR_HOME;
  process.env.MD_COMMENTS_CURSOR_HOME = home;
  try {
    return fn();
  } finally {
    if (prev === undefined) delete process.env.MD_COMMENTS_CURSOR_HOME;
    else process.env.MD_COMMENTS_CURSOR_HOME = prev;
  }
}

test('cursor discovery: walks projects/<slug>/agent-transcripts/<id>/<id>.jsonl', () => {
  const home = makeCursorHome([
    { slug: 'Users-me-repo-a', id: '11111111-1111-4111-8111-111111111111' },
    { slug: 'Users-me-repo-b', id: '22222222-2222-4222-8222-222222222222' },
  ]);
  // A session directory with no transcript must not be reported: it cannot be
  // deeplinked, rendered, or verified.
  fs.mkdirSync(path.join(home, 'projects', 'Users-me-repo-a', 'agent-transcripts', 'empty-one'), { recursive: true });
  // Sibling directories that are not transcripts at all (mcps/, canvases/).
  fs.mkdirSync(path.join(home, 'projects', 'Users-me-repo-a', 'mcps', 'some-server'), { recursive: true });

  try {
    withCursorHome(home, () => {
      // Both implementations must agree — they are a deliberate mirror.
      const fromBin = providers.providerFor('cursor').listLocal().map((s) => s.sessionId).sort();
      const fromExt = findLocalCursorSessions().map((s) => s.sessionId).sort();
      assert.deepEqual(fromBin, [
        '11111111-1111-4111-8111-111111111111',
        '22222222-2222-4222-8222-222222222222',
      ]);
      assert.deepEqual(fromExt, fromBin);
    });
  } finally {
    fs.rmSync(home, { recursive: true, force: true });
  }
...
test('cursor locate: finds a transcript in the projects tree, not just vendored', () => {
  const id = '33333333-3333-4333-8333-333333333333';
  const home = makeCursorHome([{ slug: 'Users-me-repo', id }]);
  const repo = tmpdir(); // no vendored copy here
  try {
    withCursorHome(home, () => {
      const found = providers.locateSession(`cursor:${id}`, repo);
      assert.ok(found, 'should locate via ~/.cursor/projects');
      assert.equal(found.scheme, 'cursor');
      assert.equal(path.basename(found.path), `${id}.jsonl`);
      assert.equal(locateCursorSession(id), found.path);
      // Unknown ids are a normal miss, not an error.
      assert.equal(providers.locateSession(`cursor:${'9'.repeat(8)}-0000-4000-8000-000000000000`, repo), null);
    });
  } finally {
    fs.rmSync(home, { recursive: true, force: true });
...
test('cursor state DB path is resolved per platform, not assumed to be macOS', () => {
  // The first implementation hardcoded ~/Library/Application Support, so chat
  // names silently never loaded on Linux or Windows. Failing silently is why
  // this needs a test rather than a glance.
  ...
  try {
    setPlatform('darwin');
    assert.match(cursorStateDb(), /Library\/Application Support\/Cursor\/User\/globalStorage\/state\.vscdb$/);

    setPlatform('linux');
    process.env.XDG_CONFIG_HOME = '/xdg';
    assert.equal(cursorStateDb(), path.join('/xdg', 'Cursor', 'User', 'globalStorage', 'state.vscdb'));
    delete process.env.XDG_CONFIG_HOME;
    assert.match(cursorStateDb(), /\.config\/Cursor\/User\/globalStorage\/state\.vscdb$/);

    setPlatform('win32');
    process.env.APPDATA = 'C:\\Users\\me\\AppData\\Roaming';
    assert.match(cursorStateDb(), /Cursor.User.globalStorage.state\.vscdb$/);
  } finally {
...
test('cursor chat names degrade to empty when the state DB is absent', () => {
  // Names are a convenience layer over the transcripts; a missing or
  // unreadable DB (no sqlite3 on PATH, Cursor not installed) must never throw.
  const { cursorChatNames } = require('../out/sessionProviders.js');
  const prev = process.env.MD_COMMENTS_CURSOR_STATE_DB;
  process.env.MD_COMMENTS_CURSOR_STATE_DB = path.join(tmpdir(), 'nope.vscdb');
  try {
    assert.equal(cursorChatNames().size, 0);
  } finally {
    if (prev === undefined) delete process.env.MD_COMMENTS_CURSOR_STATE_DB;
    else process.env.MD_COMMENTS_CURSOR_STATE_DB = prev;
  }
});
