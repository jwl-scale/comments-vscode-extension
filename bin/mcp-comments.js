...
function agentActor(author) {
  ...
  // Set by assign-to-Claude runs: stamps every event this server writes with
  // the driving session, so each agent reply carries its own provenance chip.
  if (process.env.MD_COMMENTS_SESSION) actor.session = process.env.MD_COMMENTS_SESSION;
  return actor;
}
...
const TOOLS = [
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        sessionId: {
          type: 'string',
          description:
            'Your agent session id, optionally scheme-qualified (claude:<sid> / codex:<sid>; bare means claude). Defaults to $MD_COMMENTS_SESSION.',
        },
    ...
    handler: (args) => {
      const rawSession = args.sessionId || process.env.MD_COMMENTS_SESSION;
  ...
  switch (method) {
    case 'initialize':
      reply({
        ...
        capabilities: { tools: {} },
        serverInfo: { name: 'anchored-comments', version: '1.0.0' },
      });
      break;
