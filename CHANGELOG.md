...
**Cursor chat names never worked off macOS.** The state DB path was hardcoded to
`~/Library/Application Support/`, so on Linux and Windows the chat picker silently fell back to raw prompt text
with no indication anything was missing. Resolved per platform now (`%APPDATA%` on Windows,
`$XDG_CONFIG_HOME`/`~/.config` elsewhere), and covered by a platform-matrix test that fails if the assumption
comes back.

**Cursor was the least-tested provider, and that was a testability defect, not an oversight.** Its paths
resolved straight off `os.homedir()`, so discovery could only be exercised against whatever happened to be on
the developer's machine — unlike Codex, which had `CODEX_HOME` from the start. With
`MD_COMMENTS_CURSOR_HOME` / `MD_COMMENTS_CURSOR_STATE_DB` overrides added, the gaps are now covered:

- **End-to-end landings** — a Cursor session verifying `agent` via `StrReplace`, via an `ApplyPatch` envelope,
  and degrading to `hybrid` when the edit went through `Shell`. Claude and Codex had these; Cursor did not.
- **Discovery** against a fixture tree, including session directories with no transcript and sibling `mcps/`
  directories, asserting the notary and extension implementations agree.
- **Locating** a transcript in the projects tree rather than only a vendored copy.
- **Graceful degradation** when the state DB or `sqlite3` is absent.
- **Extension host** coverage: the commands register, a transcript vendors and renders with its provider intact,
  and `openCursorChat` warns instead of throwing outside Cursor.
- Ref linkification for all three schemes, and thread grouping proving `claude:abc` and `cursor:abc` stay
  distinct identities rather than merging on a bare id.

## 1.2.0 — 2026-08-18

**Picking a Cursor chat, and jumping back to it.**

1.1.1 tried to bridge Cursor's "Copy Request ID" to a session id. That was the wrong bridge: only 52 of 669
stored messages carry a request id at all (14 of 16 chats), so it worked often enough to look right and failed
without explanation the rest of the time. Replaced with **Comments: Pick a Cursor Chat**, which lists chats by
the name Cursor shows in its sidebar and copies `cursor:<id>`. It reads the transcript directory — the actual
source of truth, since a chat with no transcript cannot be deeplinked anyway — and uses the state DB only to
put names on them.

**💬 Open in Cursor** closes the loop the other way. A Cursor conversation's graph now offers a button that
opens the real chat pane, via Cursor's own `composer.openComposer` command (its composerId is exactly our
session id). It appears only when it will work — a Cursor conversation viewed inside Cursor — because Cursor
registers no URL route for this, so there is nothing to fall back to in another editor. The same action is on
the chat picker.

## 1.1.1 — 2026-08-18

**Every ref-producing affordance was Claude-only.** 1.1.0 could read and render Codex and Cursor conversations
but each UI path that hands you a ref — the attach picker, the 🗂 Sessions menu, the graph's copy-deeplink
button, and the `⇥` chip on agent replies — built it by concatenating a literal `claude:`. Attaching a Cursor
session produced a ref that could never resolve. All four now take the scheme from the session, `SessionGraph`
carries the provider that produced it, and a test fails the build on any hardcoded scheme (exceptions must
declare themselves and say why).

**Finding a Cursor session id is no longer archaeology.** Cursor's UI shows a chat name and offers "Copy Request
ID", but never the session id — while the deeplink needs exactly that. Both gaps are closed by reading Cursor's
state database (readonly, via the `sqlite3` CLI, degrading silently when unavailable):

- The attach picker now labels Cursor sessions with the chat name shown in Cursor's own sidebar, rather than the
  first line of the prompt.
- **Comments: Cursor Chat Ref from Request ID** turns a copied request id into a `cursor:<id>` ref. The request
  id is not the session id — it is a per-message id recorded against the chat — so it has to be resolved rather
  than reinterpreted.

## 1.1.0 — 2026-08-18

**Cursor is a first-class provider.** Cursor's in-editor agent writes transcripts to
`~/.cursor/projects/<slug>/agent-transcripts/<id>/<id>.jsonl`, and they are now read like any other provider's:
`cursor:` refs linkify and open an interactive graph, sessions appear in discovery, the Sessions menu, and the
attach picker, transcripts are vendored on landing, and **provenance is verified** by replaying Cursor's
`Write` / `StrReplace` / `Delete` / `ApplyPatch` calls — a Cursor-authored commit can stamp `Provenance: agent`
on the same terms as Claude or Codex. Shell edits stay invisible and verify `hybrid`, as everywhere else.

Adding it required no change to anchors, event logs, the gate, landing, re-anchoring, or memory — which is what
the provider contract added in 1.0.0 was for. Two Cursor-specific wrinkles are handled in the provider and
documented in the spec:

- **No message ids.** Cursor entries carry none, so deeplink anchors are synthesized from entry position
  (`#m12`). That is sound only because the transcripts are append-only; the spec now states the rule and its
  limit, because a ref that silently retargets is worse than one that fails to resolve.
- **`ApplyPatch` is an envelope, not a diff.** It has no `@@ -a,b +c,d` line numbers and cannot be handed to
  `git apply`, so each hunk is converted to a context-matched replacement. Cursor also sometimes serializes a
  string tool input as a character-indexed object, which is reassembled before parsing.

**Cursor cannot be dispatched**, and this is not a gap that will close from our side: Cursor's agent runs inside
the editor and ships no headless CLI. `mdComments.agent.provider` therefore still offers `claude` and `codex`
only. Reading, attributing, and verifying a provider never required spawning it — the spec now says so
explicitly, so a CLI-less provider is a supported configuration rather than a degraded one.
