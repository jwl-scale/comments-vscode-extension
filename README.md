...
Highlight text in **any file**, leave an anchored comment thread, and share it with your team by committing the `.comments/` directory. Threads survive edits, refactors, and rebases; they deeplink into files (`src/foo.ts:12-34`) and into your coding agent's conversations (`claude:`, `codex:`, `cursor:` refs) rendered as an interactive graph.

Humans and agents use the same threads. A bundled MCP server lets **Claude Code** and **Codex** read, answer, and resolve comments, and **Cursor** conversations are first-class too — deeplinked, graphed, and verified for provenance; a merge queue lands agent-authored commits with **verified provenance** — the diff is replayed against the agent's own transcript, so `Provenance: agent` is a check, not a claim.
...
Cursor's in-editor agent writes transcripts to `~/.cursor/projects/<slug>/agent-transcripts/`, and Comments reads them like any other provider: `cursor:<sessionId>` refs linkify into an interactive graph, sessions appear in **🗂 Sessions…** and the attach picker, transcripts are vendored into `.comments/sessions/` on landing, and the notary verifies Cursor-authored diffs by replaying its `Write` / `StrReplace` / `ApplyPatch` / `Delete` calls — so a Cursor commit can stamp `Provenance: agent` exactly like a Claude or Codex one.

**Getting a `cursor:` ref.** Cursor's UI never shows a session id — it lives only in the transcript path (`~/.cursor/projects/<slug>/agent-transcripts/<sessionId>/`). Rather than make you find it, **Comments: Pick a Cursor Chat** lists your chats by name and copies `cursor:<id>`. (🗂 Attach Agent Session… does the same across all providers.)

**Jumping back to the live chat.** When you're running inside Cursor, a Cursor conversation's graph gets a **💬 Open in Cursor** button, and the chat picker offers the same — both call Cursor's own `composer.openComposer`, so you land in the real chat pane rather than a rendered copy. In VS Code the button is hidden, because the command doesn't exist there and Cursor exposes no URL route for it.

Two honest differences. Cursor ships **no headless CLI**, so ▶ Assign to Agent cannot dispatch it (its agent runs in the editor); Cursor sessions are read and attributed, not driven. And Cursor transcripts carry **no message ids**, so deeplink anchors are synthesized from entry position (`#m12`) — stable because the transcripts are append-only, but see [docs/spec/session-providers.md](docs/spec/session-providers.md).
...
| Comments: Pick a Cursor Chat | palette (copy a `cursor:` ref, or open the chat in Cursor) |
...
| `mdComments.agent.provider` | `claude` | `claude` \| `codex` — which agent Assign-to-Agent dispatches (Cursor has no headless CLI; see below) |
