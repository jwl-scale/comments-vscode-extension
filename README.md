...
**Getting a `cursor:` ref.** Cursor's UI never shows the session id, so there are two paths:
- **🗂 Attach Agent Session…** lists every local session with the chat name Cursor shows in its own sidebar (read from Cursor's state DB), and copies the correct `cursor:<id>` ref.
- **Comments: Cursor Chat Ref from Request ID** — paste what Cursor's *Copy Request ID* gives you. That request id is not the session id, but it is recorded against the chat, so this resolves it and copies the ref.
...
| Comments: Cursor Chat Ref from Request ID | palette (turns Cursor's "Copy Request ID" into a `cursor:` ref) |
